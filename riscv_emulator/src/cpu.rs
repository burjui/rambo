...
fn get_trap_cause(trap: &Trap, xlen: &Xlen) -> u64 {
    let interrupt_bit = match xlen {
        Xlen::Bit32 => 0x80000000,
        Xlen::Bit64 => 0x8000000000000000,
    };
...
impl Cpu {
    ...
    /// Writes integer register
    ///
    /// # Arguments
    /// * `reg` Register number. Must be 0-31
    pub fn write_register(&mut self, reg: u8, value: i64) {
        debug_assert!(reg <= 31, "reg must be 0-31. {}", reg);
    ...
    fn handle_trap(&mut self, trap: Trap, instruction_address: u64, is_interrupt: bool) -> bool {
        ...
        if is_interrupt {
            ...
            match new_privilege_encoding.cmp(&current_privilege_encoding) {
                ...
                Ordering::Equal => match self.privilege_mode {
