...
impl Memory {
    ...
    pub fn read_halfword(&self, address: u64) -> u16 {
        if (address % 2) == 0 {
            ...
            self.read_value(address, 2) as u16
        }
    }
    ...
    pub fn read_word(&self, address: u64) -> u32 {
        if (address % 4) == 0 {
            ...
            self.read_value(address, 4) as u32
        }
    }
    ...
    /// * `address`
    /// * `width` up to eight
    pub fn read_value(&self, address: u64, width: u64) -> u64 {
        let mut data = 0;
        for i in 0..width {
    ...
    pub fn write_halfword(&mut self, address: u64, value: u16) {
        if (address % 2) == 0 {
            ...
            self.write_value(address, value as u64, 2);
        }
    }
    ...
    pub fn write_word(&mut self, address: u64, value: u32) {
        if (address % 4) == 0 {
            ...
            self.write_value(address, value as u64, 4);
        }
    }
    ...
    pub fn write_doubleword(&mut self, address: u64, value: u64) {
        if (address % 8) == 0 {
            ...
            self.write_value(address, value, 8);
        }
    }
    ...
    /// * `value`
    /// * `width` up to eight
    pub fn write_value(&mut self, address: u64, value: u64, width: u64) {
        for i in 0..width {
            self.write_byte(address.wrapping_add(i), (value >> (i * 8)) as u8);
        }
    }

    pub fn write_bytes(&mut self, address: u64, s: &[u8]) {
        for (i, byte) in s.iter().cloned().enumerate() {
